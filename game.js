...
const GAME_KEYS = [
  {
    id: "key1",
    x: 3850,
    title: "Key 1 — Temple Builder Unlocked",
    text: "You found the first heritage key! The Temple Builder challenge is now unlocked."
  },
  {
    id: "key2",
    x: 7000,
    title: "Key 2 — Heritage Quiz Unlocked",
    text: "You found the second heritage key! The Heritage Quiz challenge is now unlocked."
  }
];
...
function updateCollectionHUD() {
  ...
  if (count) count.textContent = `${discoveredHeritage.size + collectedKeys.size} / 6 COLLECTED`;
}
...
function showFinalCollection() {
  ...
  GAME_KEYS.forEach(item => {
    const card = document.createElement("article");
    card.className = "collection-card key-card";
    card.innerHTML = `<div class="collection-icon">🔑</div>
      <div class="collection-content">
        <div class="collection-status">✓ COLLECTED</div>
        <h3>${item.title}</h3>
        <p>${item.text}</p>
        <div class="collection-fact"><strong>UNLOCKED</strong> This key is ready for its next heritage game.</div>
      </div>`;
    grid.appendChild(card);
  });
...
function update(dt) {
  ...
  const allKeysCollected = collectedKeys.size >= GAME_KEYS.length;

  if (state.score >= TARGET_SCORE && allHeritageCollected && allKeysCollected) {
    completeLevel();
    return;
  }
}
...
function updateKeyHUD() {
  ...
  if (k1) {
    const found = collectedKeys.has("key1");
    k1.classList.toggle("unlocked", found);
    k1.classList.toggle("locked", !found);
    k1.querySelector("span").textContent = found ? "FOUND ✓" : "LOCKED";
  }
  if (k2) {
    const found = collectedKeys.has("key2");
    k2.classList.toggle("unlocked", found);
    k2.classList.toggle("locked", !found);
    k2.querySelector("span").textContent = found ? "FOUND ✓" : "LOCKED";
  }
}
...
function checkGameKeys() {
  ...
  for (const item of GAME_KEYS) {
    if (collectedKeys.has(item.id)) continue;
    const screenX = item.x - scroll;
    if (Math.abs(screenX - playerScreenX) < 95) {
      collectGameKey(item);
      break;
...
function collectGameKey(item) {
  collectedKeys.add(item.id);
  activeKey = null;
  keyPaused = false;
  heritagePaused = false;
  updateKeyHUD();
...
function continueFromKey() {
  keyPaused = false;
  activeKey = null;
  document.getElementById("keyMessageOverlay")?.classList.add("hidden");
}



function drawGameKeys() {
  const scroll = Number.isFinite(state.worldX) ? state.worldX : 0;
  ...
  for (const item of GAME_KEYS) {
    ...
    const x = item.x - scroll;
    if (x < -100 || x > innerWidth + 100) continue;

    const baseY = groundY() - 12;
    const bob = Math.sin(performance.now() / 260 + item.x) * 5;

    ctx.save();
    ctx.textAlign = "center";
    ctx.font = "40px serif";
    ctx.shadowColor = "rgba(255, 211, 80, .95)";
    ctx.shadowBlur = 20;
    ctx.fillText("🔑", x, baseY - 55 + bob);
    ctx.shadowBlur = 0;
    ctx.font = "800 11px Arial, sans-serif";
    ctx.fillStyle = "#ffe7a6";
    ctx.fillText(item.id === "key1" ? "KEY 1" : "KEY 2", x, baseY - 10 + bob);
    ctx.strokeStyle = "rgba(255, 220, 130, .6)";
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.arc(x, baseY - 42 + bob, 30, 0, Math.PI * 2);
    ctx.stroke();
    ctx.restore();
  }
}
