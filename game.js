...
function resize() {
  ...
  // Scale the logical player to the viewport so the game remains comfortable
  // on phones, tablets and laptops while preserving desktop proportions.
  const compact = Math.min(innerWidth, innerHeight) < 600;
  player.w = compact ? 58 : (innerWidth < 900 ? 64 : 72);
  player.h = compact ? 92 : (innerWidth < 900 ? 102 : 112);

  player.x = Math.max(compact ? 48 : 70, innerWidth * 0.14);
...
canvas.addEventListener("pointerup", e => {
  ...
  // Swipe controls make the game playable without a keyboard.
  if (elapsed < 500 && Math.abs(dy) > 35 && Math.abs(dy) > Math.abs(dx)) {
    if (dy < 0) jump();
    else slide();
    return;
  }

  // A simple tap: upper half/right side jumps, lower-left side slides.
